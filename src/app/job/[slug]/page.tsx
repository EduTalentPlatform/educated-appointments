...
export default async function JobPage({ params }: Props) {
  ...
  // Tier 4: Related role type group (commercial with commercial, delivery with delivery)
  // Prevents Sales roles appearing next to Tutoring/MIS etc.
  const DELIVERY_ROLES = ['Assessing', 'Tutoring & Teaching', 'Skills Coaching', 'Functional Skills', 'Curriculum & Leadership']
  const COMMERCIAL_ROLES = ['Sales & Business Development', 'Employer Engagement']
  const OPERATIONS_ROLES = ['MIS & Data', 'Operations & Management']
  const LEADERSHIP_ROLES = ['Senior Management', 'Director', 'Executive Leadership']

  const getGroup = (sector: string) => {
    if (DELIVERY_ROLES.includes(sector)) return DELIVERY_ROLES
    if (COMMERCIAL_ROLES.includes(sector)) return COMMERCIAL_ROLES
    if (OPERATIONS_ROLES.includes(sector)) return OPERATIONS_ROLES
    if (LEADERSHIP_ROLES.includes(sector)) return LEADERSHIP_ROLES
    return null
  }

  if (similar.length < 3) {
    const group = getGroup(job.sector)
    if (group) {
  ...
  // Tier 5: Absolute last resort — any live role
  // Only reached if the group didn't fill 3 slots (e.g. very few live roles)
  if (similar.length < 3) {
